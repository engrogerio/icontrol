...
class IFormTagForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        """
        This will show just unused tags to choose to insert on a iform.
        """
        iform_tag = kwargs.pop('instance')
        super(IFormTagForm, self).__init__(*args, **kwargs)
        # these are the tags of the current form
        iform_tags = IFormTag.objects.all().filter(id=iform_tag.id).values_list('tag').distinct()
        # these are all already assigned tags including those from the current form
        assigned_tags = IFormTag.objects.all().values_list('tag').distinct()
        #assigned_tags = assigned_tags | iform_tags
        #self.fields['tag'].queryset = Tag.objects.exclude(id__in=assigned_tags).order_by('name')
